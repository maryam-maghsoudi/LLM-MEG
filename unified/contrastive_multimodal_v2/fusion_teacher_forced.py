...
def meg_scores_to_type_level(z_word, bank_vectors, bank_word_types, vocab):
    ...
    """
    # v2: WordProjectionHead output is NOT L2-normalized (unlike v1), so
    # normalize BOTH sides here for true cosine similarity — consistent with
    # eval_stage1.py's cosine_similarity_matrix and fusion_beamsearch.py.
    z_norm    = F.normalize(z_word.float(), dim=-1)         # (N, d)
    bank_norm = F.normalize(bank_vectors.float(), dim=-1)   # (117, d)
    sim = (z_norm @ bank_norm.T).cpu()                     # (N, 117)
...
def main(args):
    ...
    bank_vectors, _, bank_word_types, type_to_id, _, _ = build_candidate_bank(
