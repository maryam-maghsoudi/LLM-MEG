...
def build_candidate_bank(teacher_cache, gpt2_mid_layer=None):
    """
    Build the h_mid occurrence bank from teacher_cache.pt.
    Returns (bank_vectors, bank_word_types, type_to_id).

    gpt2_mid_layer : if given, build the bank from
        cache["hidden_states_word_aligned"][gpt2_mid_layer] instead of
        cache["h_mid"]. Must match whatever GPT-2 layer the Stage 1 encoder
        was trained against (train.py --gpt2_mid_layer). None → cache["h_mid"]
        (the default layer, matching default-trained checkpoints).
    """
    all_vecs, all_types = [], []
    ...
    for poem_key, cache in teacher_cache.items():
        if not isinstance(cache, dict) or "h_mid" not in cache:
            continue
        if gpt2_mid_layer is not None:
            h_mid = cache["hidden_states_word_aligned"][gpt2_mid_layer]  # (N_words, d)
        else:
...
def main(args):
    ...
    bank_vectors, bank_word_types, type_to_id = build_candidate_bank(
