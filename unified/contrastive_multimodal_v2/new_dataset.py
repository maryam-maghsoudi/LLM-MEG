...
def _load_meg_trial(
    ...
    If a preprocessed .npy file exists in meg_base, it is
    ...
    Parameters
    ----------
    meg_base : override for MEG_BASE   None → MEG_BASE.
    """
    base    = Path(meg_base) if meg_base is not None else MEG_BASE
    meg_dir = base / subject / f"ses-{session}" / "meg"

    # preprocessed .npy (produced by export_preprocessed.py)
...
class MEGContinuousTrialDataset(Dataset):
    ...
    trial, per-word onset/offset sample
