...
(subject, poem, session) go exclusively to one partition.
"""
...
POEM_KEYS:  List[str] = ["poem1", "poem2"] # Poem stimuli
N_SESSIONS: int       = 10                 # Number of sessions per subject

# Word positions per line (1-indexed lines), derived from WhisperX forced
# alignment using a 0.8 s gap threshold.
...
def make_loso_splits(
    ...
    Leave-one-subject-out.
      train : 11 non-heldout subjects × both poems × sessions 0–9
      val : heldout val subject x both poems x sessions 0-9 
      test  : heldout test subject × both poems × sessions 0–9
    """
    if val_subject is None:
        val_subject = next(s for s in all_subjects if s != heldout_subject)
    if val_subject == heldout_subject:
        raise ValueError("val_subject must differ from heldout_subject")
    if heldout_subject not in all_subjects:
        raise ValueError(
            f"{heldout_subject!r} not in SUBJECTS list.\nValid: {all_subjects}"
        )
    train_subs = [s for s in all_subjects if s not in ( heldout_subject, val_subject )]
    splits = {
        "train": {"trials": _trials(train_subs,        list(range(N_SESSIONS))), "word_filter": None},
        "val":   {"trials": _trials([val_subject],     list(range(N_SESSIONS))), "word_filter": None},
        "test":  {"trials": _trials([heldout_subject], list(range(N_SESSIONS))), "word_filter": None},
    }
    print(f"LOSO split  heldout={heldout_subject} val_subject = {val_subject}")
